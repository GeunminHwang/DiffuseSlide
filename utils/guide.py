...
class MSEGuidance(Guidance):
    ...
    def _forward(self, target_x0: torch.Tensor, pred_x0: torch.Tensor, t: int) -> Tuple[torch.Tensor, float]:
        # inputs: [-1, 1], bnchw (batch, frames, channels, height, width)
        with torch.enable_grad():
            pred_x0.requires_grad_(True)
            # 프레임 축까지 포함하여 손실 계산
        ...
        # temporal_loss = 0
        # for i in range(pred_x0.shape[1] - 1):
        #     temporal_loss += ((pred_x0[:, i] - pred_x0[:, i + 1]).pow(2).mean((1, 2, 3, 4)))
            
        # total_loss = loss + lambda_temporal * temporal_loss
...
#         with torch.enable_grad():
#             for frame_idx in range(pred_x0.size(1)):  
#                 pred_frame = pred_x0[:, frame_idx].detach().clone().requires_grad_(True)
#                 target_frame = target_x0[:, frame_idx]
                
#                 # 프레임별 MSE 손실 계산
#                 frame_loss = (pred_frame - target_frame).pow(2).mean((1, 2, 3)).sum()
#                 loss_vals.append(frame_loss.item())
                
#                 # 프레임별 gradient 계산
#                 frame_gradient = -torch.autograd.grad(frame_loss, pred_frame)[0] * self.scale
#                 gradients.append(frame_gradient)
        
#         # 전체 손실 계산 (프레임별 손실의 합)
#         total_loss = sum(loss_vals)
        
#         # 프레임별 gradient를 하나의 텐서로 결합 (원래 pred_x0 형태로)
#         gradient_tensor = torch.stack(gradients, dim=1)  # (batch, frames, channels, height, width)
        
#         return gradient_tensor, total_loss
